...
<!DOCTYPE html>
  <html>
    ...
    <title>Cadastro de Letras - Divulga Músicas</title>
...
if(isset($_POST['cadastrar'])){
		...
		$file 		= $_FILES['partitura'];
		$numFile	= count(array_filter($file['name']));

		$folder		= 'arquivos/imagens/';
		$maxSize	= 1024 * 1024 * 3;
		...
		if($numFile <= 0){
			echo '<div class="alert alert-danger">
						...
						É necessário que você escolha uma imagem para continuar!
					</div>';
		}
		else if($numFile >=20){
			echo '<div class="alert alert-danger">
						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						A imagem que você escolheu ultrapassa nosso limite de upload. Selecione apenas uma imagem e tente novamente!
					</div>';
		}else{
			for($i = 0; $i < $numFile; $i++){
				$name 	= $file['name'][$i];
...
$insert = "INSERT into musica (titulo, autor, data, letra, partitura, fins, genero) VALUES (:titulo, :autor, :data, :letra, :partitura, :fins, :genero)";
					...
					try{
                            ...
                            $result->bindParam(':letra', $letra, PDO::PARAM_STR);
							$result->bindParam(':partitura', $novoNome, PDO::PARAM_STR);
	 ...
	 ?>
        ...
        <br>
        <div class="container">
            <h1>Cadastre sua música (Letra):</h1>
  <form class="col s12" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="row">
        <div class="input-field col s12">
          ...
          <label for="<?php echo $nome; ?>">Autor</label>
        </div>
      </div>
      <!--Data-->
      ...
      <div class="input-field col s12">
      <textarea name="letra" id="editor1" rows="10" cols="80">
