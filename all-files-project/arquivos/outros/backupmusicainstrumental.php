...
//INFO MUSIC DEMO
        ...
        $fileMusicaCompleta = $_FILES['musica_completa'];
        $filePartirura = $_FILES['partitura'];
		$numFile	= count(array_filter($file['name']));
        //$numFileM	= count(array_filter($fileMusicaCompleta['name']));
        //$numFileP	= count(array_filter($filePartirura['name']));
		
		//PASTA
		$folder		= 'arquivos/musicas/';
        $maxSize	= 1024 * 500000;
    
        $folderM	= 'arquivos/musicas/';
        $maxSizeM	= 1024 * 500000;
    
        $folderPartitura		= 'arquivos/imagens/';
		$maxSizePartitura	= 1024 * 1024 * 3;
		
		//REQUISITOS
		$permite 	= array('audio/wav', 'audio/wave','audio/mp3');
        $permiteM 	= array('audio/wav', 'audio/wave','audio/mp3');
        $permitePartitura 	= array('image/jpeg', 'image/png');
		...
		if($numFile <= 0){
			...
			for($i = 0; $i < $numFile; $i++){
                ...
                //musica completa
                $nameM 	= $fileMusicaCompleta['name'][$i];
				$typeM	= $fileMusicaCompleta['type'][$i];
				$sizeM	= $fileMusicaCompleta['size'][$i];
				$errorM	= $fileMusicaCompleta['error'][$i];
				$tmpM	= $fileMusicaCompleta['tmp_name'][$i];
                
                //partitura
                $nameP 	= $filePartirura['name'][$i];
				$typeP	= $filePartirura['type'][$i];
				$sizeP	= $filePartirura['size'][$i];
				$errorP	= $filePartirura['error'][$i];
				$tmpP	= $filePartirura['tmp_name'][$i];
                ...
                //musica completa
                $extensaoM = @end(explode('.', $nameM));
				$novoNomeM = rand().".$extensaoM";
                
                //partitura
                $extensaoP = @end(explode('.', $nameP));
				$novoNomeP = rand().".$extensaoP";
                
				if($error != 0)
					echo $msg[] = "<b>$name :</b> ".$errorMsg[$error];
				else if(!in_array($type, $permite) && !in_array($typeM, $permiteM) && !in_array($typeP, $permitePartitura))
					echo $msg[] = "<b>$name :</b> Erro áudio não suportado!";
				else if($size > $maxSize && $sizeM > $maxSizeM && $sizeP > $maxSizePartitura)
					echo $msg[] = "<b>$name :</b> Erro áudio ultrapassa o limite de 5MB";
				else{
					
					if(move_uploaded_file($tmp, $folder.'/'.$novoNome) && move_uploaded_file($tmpM, $folder.'/'.$novoNomeM) && move_uploaded_file($tmpP, $folderPartitura.'/'.$novoNomeP) ){
...
$insert = "INSERT into musicainst (titulo, autor, data, musica_demo, fins, genero) VALUES (:titulo, :autor, :data, :musica_demo, :fins, :genero)";
					...
					try{
                            ...
                            $result->bindParam(':musica_completa', $novoNomeM, PDO::PARAM_STR);
							$result->bindParam(':partitura', $novoNomeP, PDO::PARAM_STR);
							...
							if($contar>0){
								...
								echo '<div class="card-panel #d50000 red accent-2"><b>Erro ao cadastrar!</strong> Não foi possível cadastrar sua música.</b></div>';
							}			
						}catch(PDOException $e){
							echo $e;
						}	
